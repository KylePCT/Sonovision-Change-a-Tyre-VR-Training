...
namespace Photon.Voice.Unity
{
    ...
    public class UnityAndroidAudioInAEC : Voice.IAudioPusher<short>
    {
        ...
        public UnityAndroidAudioInAEC(Voice.ILogger logger, bool enableAEC = false, bool enableAGC = false, bool enableNS = false)
        {
            ...
            try
            {
                ...
                logger.LogInfo("[PV] UnityAndroidAudioInAEC: AndroidJavaObject created: aec: {0}/{1}, agc: {2}/{3}, ns: {4}/{5} minBufSize: {6}", 
                ...
                if (ok)
                {
                    ...
                    logger.LogInfo("[PV] UnityAndroidAudioInAEC: AndroidJavaObject started: {0}, sampling rate: {1}, channels: {2}, record buffer size: {3}", ok, SamplingRate, Channels, minBufSize * 4);
                }
                else
                {
            ...
            catch (Exception e)
            {
                ...
                {
                    Error = "Exception in UnityAndroidAudioInAEC constructor";
                }
                logger.LogError("[PV] UnityAndroidAudioInAEC: {0}", Error);
            }
        }
